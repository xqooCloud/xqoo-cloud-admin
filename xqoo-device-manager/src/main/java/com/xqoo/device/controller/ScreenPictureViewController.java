...
@RestController
@RequestMapping("/screenPictureViewHandle")
@Api(tags = "屏幕预览图片表控制器")
@Validated
public class ScreenPictureViewController{
    ...
    */
    ...
    @ApiOperation("分页获取屏幕预览图片表表数据")
    @PostMapping("/pageGetList")
    public ResultEntity<PageResponseBean<ScreenPictureViewEntity>> pageGetList(@RequestBody PageRequestBean page){
        return screenPictureViewService.pageGetList(page);
    }

    @ApiOperation("批量新增数据")
    @PostMapping("/addRecordByList")
    @OperationLog(tips="批量新增screen_picture_view表数据", operatorType = OperationTypeEnum.ADD, isSaveRequestData = true)
    public ResultEntity<String> addRecordByList(@ApiIgnore @LoginUser CurrentUser currentUser,
                                        @RequestBody List<ScreenPictureViewEntity> list){
        if(StringUtils.isEmpty(currentUser.getUserId())){
            return new ResultEntity<>(HttpStatus.NOT_ACCEPTABLE, "未找到当前登录人信息，请重新登录重试");
        }
        return screenPictureViewService.insertList(list, currentUser);
    }

    @ApiOperation("根据主键查询单条记录")
    @GetMapping("/getRecordByPrimaryKey")
    public ResultEntity<ScreenPictureViewEntity> getRecordByPrimaryKey(@RequestParam(required = false, value = "id")
                                                                      @NotNull(message = "主键值不能为空") Long id){
        ScreenPictureViewEntity entity = screenPictureViewService.getOneScreenPictureViewEntityByPrimaryKey(id);
