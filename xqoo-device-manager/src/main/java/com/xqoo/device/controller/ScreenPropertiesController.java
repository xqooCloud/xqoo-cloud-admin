...
@RestController
@RequestMapping("/screenPropertiesHandle")
@Api(tags = "屏幕信息参数表控制器")
@Validated
public class ScreenPropertiesController{
    ...
    */
    ...
    @ApiOperation("分页获取屏幕信息参数表表数据")
    @PostMapping("/pageGetList")
    public ResultEntity<PageResponseBean<ScreenPropertiesEntity>> pageGetList(@RequestBody PageRequestBean page){
        return screenPropertiesService.pageGetList(page);
    }

    @ApiOperation("批量新增数据")
    @PostMapping("/addRecordByList")
    @OperationLog(tips="批量新增screen_properties表数据", operatorType = OperationTypeEnum.ADD, isSaveRequestData = true)
    public ResultEntity<String> addRecordByList(@ApiIgnore @LoginUser CurrentUser currentUser,
                                        @RequestBody List<ScreenPropertiesEntity> list){
        if(StringUtils.isEmpty(currentUser.getUserId())){
            return new ResultEntity<>(HttpStatus.NOT_ACCEPTABLE, "未找到当前登录人信息，请重新登录重试");
        }
        return screenPropertiesService.insertList(list, currentUser);
    }

    @ApiOperation("根据主键查询单条记录")
    @GetMapping("/getRecordByPrimaryKey")
    public ResultEntity<ScreenPropertiesEntity> getRecordByPrimaryKey(@RequestParam(required = false, value = "id")
                                                                      @NotNull(message = "主键值不能为空") Long id){
        ScreenPropertiesEntity entity = screenPropertiesService.getOneScreenPropertiesEntityByPrimaryKey(id);
