...
/**
 * CommentContenInfoController
 *
 * @author xqoo-code-gen
 * @date 2021-02-01 14:48:39
 */

@RestController
@RequestMapping("/commentContenInfoHandle")
@Api(tags = "评论信息表控制器")
@Validated
public class CommentContentInfoController{
    ...
    @Autowired
    private CommentContentInfoService commentContentInfoService;


    @ApiOperation("分页获取评论信息表表数据")
    @PostMapping("/pageGetList")
    public ResultEntity<PageResponseBean<CommentContentInfoEntity>> pageGetList(@RequestBody PageRequestBean page){
        return commentContentInfoService.pageGetList(page);
    }

    @ApiOperation("批量新增数据")
    @PostMapping("/addRecordByList")
    @OperationLog(tips="批量新增comment_conten_info表数据", operatorType = OperationTypeEnum.ADD, isSaveRequestData = true)
    public ResultEntity<String> addRecordByList(@ApiIgnore @LoginUser CurrentUser currentUser,
                                                @RequestBody List<CommentContentInfoEntity> list){
        if(StringUtils.isEmpty(currentUser.getUserId())){
            return new ResultEntity<>(HttpStatus.NOT_ACCEPTABLE, "未找到当前登录人信息，请重新登录重试");
        }
        return commentContentInfoService.insertList(list, currentUser);
    }

    @ApiOperation("根据主键查询单条记录")
    @GetMapping("/getRecordByPrimaryKey")
    public ResultEntity<CommentContentInfoEntity> getRecordByPrimaryKey(@RequestParam(required = false, value = "id")
                                                                       @NotNull(message = "主键值不能为空") Long id){
        CommentContentInfoEntity entity = commentContentInfoService.getOneCommentContenInfoEntityByPrimaryKey(id);
