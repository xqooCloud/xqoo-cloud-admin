...
@RestController
@RequestMapping("/goodsCommentHandle")
@Api(tags = "商品评价表控制器")
@Validated
public class GoodsCommentController{
    ...
    */
    ...
    @ApiOperation("分页获取商品评价表表数据")
    @PostMapping("/pageGetList")
    public ResultEntity<PageResponseBean<GoodsCommentEntity>> pageGetList(@RequestBody PageRequestBean page){
        return goodsCommentService.pageGetList(page);
    }

    @ApiOperation("批量新增数据")
    @PostMapping("/addRecordByList")
    @OperationLog(tips="批量新增goods_comment表数据", operatorType = OperationTypeEnum.ADD, isSaveRequestData = true)
    public ResultEntity<String> addRecordByList(@ApiIgnore @LoginUser CurrentUser currentUser,
                                        @RequestBody List<GoodsCommentEntity> list){
        if(StringUtils.isEmpty(currentUser.getUserId())){
            return new ResultEntity<>(HttpStatus.NOT_ACCEPTABLE, "未找到当前登录人信息，请重新登录重试");
        }
        return goodsCommentService.insertList(list, currentUser);
    }

    @ApiOperation("根据主键查询单条记录")
    @GetMapping("/getRecordByPrimaryKey")
    public ResultEntity<GoodsCommentEntity> getRecordByPrimaryKey(@RequestParam(required = false, value = "commentId")
                                                                      @NotNull(message = "主键值不能为空") String commentId){
        GoodsCommentEntity entity = goodsCommentService.getOneGoodsCommentEntityByPrimaryKey(commentId);
