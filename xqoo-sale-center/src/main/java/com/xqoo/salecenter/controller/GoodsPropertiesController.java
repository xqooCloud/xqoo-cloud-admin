...
@RestController
@RequestMapping("/goodsPropertiesHandle")
@Api(tags = "商品信息参数表控制器")
@Validated
public class GoodsPropertiesController{
    ...
    */
    ...
    @ApiOperation("分页获取商品信息参数表表数据")
    @PostMapping("/pageGetList")
    public ResultEntity<PageResponseBean<GoodsPropertiesEntity>> pageGetList(@RequestBody PageRequestBean page){
        return goodsPropertiesService.pageGetList(page);
    }

    @ApiOperation("批量新增数据")
    @PostMapping("/addRecordByList")
    @OperationLog(tips="批量新增goods_properties表数据", operatorType = OperationTypeEnum.ADD, isSaveRequestData = true)
    public ResultEntity<String> addRecordByList(@ApiIgnore @LoginUser CurrentUser currentUser,
                                        @RequestBody List<GoodsPropertiesEntity> list){
        if(StringUtils.isEmpty(currentUser.getUserId())){
            return new ResultEntity<>(HttpStatus.NOT_ACCEPTABLE, "未找到当前登录人信息，请重新登录重试");
        }
        return goodsPropertiesService.insertList(list, currentUser);
    }

    @ApiOperation("根据主键查询单条记录")
    @GetMapping("/getRecordByPrimaryKey")
    public ResultEntity<GoodsPropertiesEntity> getRecordByPrimaryKey(@RequestParam(required = false, value = "id")
                                                                      @NotNull(message = "主键值不能为空") Long id){
        GoodsPropertiesEntity entity = goodsPropertiesService.getOneGoodsPropertiesEntityByPrimaryKey(id);
