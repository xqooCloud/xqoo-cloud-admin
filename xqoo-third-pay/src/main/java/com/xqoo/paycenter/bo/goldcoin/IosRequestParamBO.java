...
public class IosRequestParamBO {
    ...
    @ApiModelProperty(value = "用户充值的金币数",required = true)
    ...
    public boolean equals(Object o) {
        ...
        return new EqualsBuilder().append(transactionId, that.transactionId).append(userId, that.userId).append(applePayLoad, that.applePayLoad).append(orderCode, that.orderCode).append(orderAmount, that.orderAmount).isEquals();
    }

    @Override
    public int hashCode() {
        return new HashCodeBuilder(17, 37).append(transactionId).append(userId).append(applePayLoad).append(orderCode).append(orderAmount).toHashCode();
    }
}
